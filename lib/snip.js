...
function snip(sourceID, manifest, timeOpts, opts, handler){
    ...
    snipData.version = snipData.version ? ++snipData.version : 1;
    ...
    return async.waterfall([
        ...
        function(next){
            // Edit in place
            if(opts.sameFile){
                log.info("Using existing snippet, not re-extracting  ...");
                return fs.copy(snipData.paths.mp3, snipData.paths.tmp, next);
            }
            // Extract from source
            else {
                if(typeof timeOpts.from === 'undefined' && typeof timeOpts.duration === 'undefined'){
                    if(snipData.startsAt){ timeOpts.from     = snipData.startsAt; }
                    if(snipData.length)  { timeOpts.duration = snipData.length+5;   }
                }
                ...
                log.info("Extracting snippet from original source '"+snipData.sourceID+"' ...");
                 return roughCutSnippet(source.path, snipData.paths.tmp, timeOpts, next);
            }
        },
